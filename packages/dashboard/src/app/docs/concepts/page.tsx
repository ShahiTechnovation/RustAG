...
const TOC: TocItem[] = [
  ...
  { id: "svm-replay", title: "Why this matters on the EVM", depth: 3 },
...
const MATRIX = [
  { tool: "robinhood chain-test-validator", cells: [false, false, false, false, false], highlight: false },
  { tool: "EVM sandbox / Bankrun (libs)", cells: [false, false, false, false, false], highlight: false },
...
Background: re-fetch Clean ORACLE accounts every 30s
      ...
      <p>
        ...
        (the default interval), so Oracle prices stay fresh. In the transaction path, a pre-load
        step batch-fetches any static account key that is not already loaded and not <code>Dirty</code>,
        loading it into EVM sandbox as <code>Clean</code>; fetch failures are logged and tolerated.
      </p>

      <H3 id="svm-replay">Why this matters on the EVM</H3>
      <p>
        This is how &ldquo;mainnet replay&rdquo; works on Robinhood Chain. EVM tools (Tenderly, Anvil&apos;s{" "}
        <code>--fork-url</code>) fork at a block hash and pull state from that fixed point; the EVM has no
      ...
      <p>
        ...
        doesn&apos;t fork the Robinhood Chain crate versions EVM sandbox 0.12 unifies on.
      </p>
      <Callout variant="early" title="Known limitation (early access)">
        ...
        deployed program can read real mainnet state today; invoking a foreign program like a full Uniswap
    ...
    Pinned,
      ...
      <p>
        ...
        interval (clamped to a 1s minimum), so Oracle prices don&apos;t go stale under your tests.
      </p>
      <p>
