        ...
        rows={[
          {
            ...
            type: "—",
            desc: "multisig VaultTransaction proposal address. RustAG fetches, Borsh-decodes, and rehearses it.",
          },
          {
            name: "--payload <BASE64>",
            type: "—",
          ...
          },
          {
            name: "--signer <PATH>",
            type: "ephemeral",
            desc: "Path to a Robinhood Chain JSON keypair (64-byte array) to sign the bundle with.",
          },
          {
            name: "--out <PATH>",
...
rustag forensics <SIGNATURE> \\
  ...
  --patch-program <PROGRAM_ID>`}
      ...
      <Callout variant="info">
        These commands manage a persistent local EVM stagenet — useful for integration testing
...
rustag start demo --preload oracle dex`}
      ...
      <p>
        Credit ETH to a wallet via the running stagenet. Airdrops are unlimited — capped only
...
rustag verify .rustag/demo.attestation.json -s demo`}
      ...
      <Callout variant="info">
        ...
        contract, and the Robinhood Chain JSON-RPC methods the stagenet implements.
      </Callout>
    </DocArticle>
  );
}
