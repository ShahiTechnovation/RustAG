...
const FAQ = [
  ...
  },
  {
    ...
    a: "Phase 1 ships 6 rules: upgrade_authority_changed (CRITICAL), large_sol_drain (HIGH, >100 ETH delta), nonce_authority_combo (MEDIUM — nonce + authority rotation in same tx), program_freeze_guard (HIGH), token_authority_changed (HIGH), account_closed_drain (MEDIUM). More rules are additive in future phases.",
  },
  {
  ...
  },
  {
    ...
    a: "No. Everything described in Phase 1 runs entirely from source — cargo build --release, then rustag rehearse. The hosted service (Phase 2+) adds the Evidence Registry, multisig UI embed, and N-of-M provenance chain. The CLI + local server is and will remain open source.",
  },
];
...
assert!(report.alarms.is_empty() || report.alarms.iter().all(|a| a.severity < Severity::High));`}
      ...
      <ol>
        ...
        <li>Execute the payload in a fresh EVM sandbox instance loaded from the closure.</li>
        <li>
      ...
      <p>
        ...
        anchor a verifier needs to establish out-of-band (e.g., from the multisig multisig&apos;s
...
# ✓ Grade A · Signature valid · pre/post roots match`}
      ...
      <Callout variant="early" title="Phase 1 honest boundary">
        ...
        The Evidence Registry (hosted append-only bundle store), multisig UI embed (signer-review
