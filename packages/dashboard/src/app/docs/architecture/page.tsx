...
export const metadata: Metadata = {
  title: "Architecture",
  description:
    "RustAG dual-layer design: Ingest layer (TouchSetResolver, SquadsDecoder, MultiRpcFetcher) feeding a Sealed Rehearsal layer (EVM sandbox, semantic diff, invariant policy, EvidenceBundle signing).",
};
...
const DATA_FLOW = `  Wallet / multisig UI / Multisig signer / CI pipeline
...
│   Pass 1 (Pre-state)                                     │
│     • load closure into isolated EVM sandbox instance        │
...
const CRATES: { crate: string; resp: string; phase: string; tone: Tone }[] = [
  ...
  {
    crate: "rustag-core",
    resp: "Persistent EVM stagenet runtime: EVM sandbox + AccountSync state machine (Unknown→Clean→Dirty→Pinned) + SQLite via sqlx.",
    phase: "1",
    tone: "lime",
  },
  {
    crate: "rustag-rpc",
    resp: "axum server: POST /api/rehearse, POST /api/verify, Robinhood Chain-compatible JSON-RPC, WebSocket, REST API.",
    phase: "1",
    tone: "lime",
  },
  {
...
export default function ArchitecturePage() {
  return (
    ...
    >
      ...
      <ul>
        <li>
          <strong>SquadsDecoder</strong> — Borsh-decodes a multisig{" "}
        ...
        <li>
          ...
          keeps EVM sandbox 0.12 dep compatibility.
        </li>
        <li>
      ...
      <ol>
        <li>
          <strong>Pass 1 — Pre-state root</strong>: load the closure into an isolated EVM sandbox
      ...
      <ul>
        ...
        <li>
          ...
          N-of-M signer provenance. A multisig vault can require M-of-N reviewers to submit a valid
          Grade A bundle before a proposal can be approved.
        </li>
        <li>
          <strong>multisig web UI embed</strong> — a signer-review panel that fetches and renders
          the EvidenceBundle inline in the multisig proposal UI, without requiring any CLI.
        </li>
        <li>
...
// + 5 more: Created, Frozen, Thawed, NonceDerived, SysvarMutated`}
      ...
      <Callout variant="early" title="Honest boundary">
        ...
        The Evidence Registry, multisig UI embed, Yellowstone gRPC, and hosted multi-tenant service are
