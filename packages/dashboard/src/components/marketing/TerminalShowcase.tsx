...
export function TerminalShowcase() {
  return (
    <Section
      ...
      description="Paste a multisig proposal address — RustAG fetches it, rehearses it against live mainnet state, and returns a signed EvidenceBundle you can verify offline."
      containerClassName="max-w-3xl"
    >
      <Reveal>
        <Terminal title="bash" copyText={COPY_TEXT}>
          <pre className="whitespace-pre text-xs leading-relaxed sm:text-sm">
            <span className={C.comment}># rehearse a multisig multisig proposal{"\n"}</span>
